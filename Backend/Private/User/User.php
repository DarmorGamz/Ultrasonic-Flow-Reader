...
final class User {
    ...
    final public function UserGsad(string $sCmd, array $aVarsIn, array &$aVarsOut): bool {
        ...
        if("User.Get" == $sCmd) {
            ...
            // Get User.
            if(!$this->oDb->UserGsad("Login", $aVarsIn1, $aVarsOut1)) { /* Error */ return false; }

            // Set Response.
            $aVarsOut = $aVarsOut1;
        } else if("ResetPassword" == $sCmd) {
            // Init vars
            $aVarsIn1 = $aVarsIn; $aVarsOut1 = [];

            // Get User.
            if(!$this->oDb->UserGsad("ResetPassword", $aVarsIn1, $aVarsOut1)) { /* Error */ return false; }
...
class UserDb extends Db {
    ...
    final public function UserGsad(string $sCmd, array $aVarsIn, array &$aVarsOut): bool {
        ...
        if("Get" == $sCmd) {
            ...
            // Add User.
            $sQry = "INSERT INTO User (Email, PwdMd5) VALUES ('{$sEmail}', '{$sPassword}')";
            if(($oRes = $this->oDb->query($sQry)) === false) { /* Error */ return false; }
            ...
            // Init vars.
            if(empty($aVarsIn['Email']) || empty($aVarsIn['PwdMd5']) || empty($aVarsIn['PwdOldMd5'])) { /* Error */ return false;}
            $sEmail = $aVarsIn['Email']; $sPasswordOld = $aVarsIn['PwdOldMd5']; $sPassword = $aVarsIn['PwdMd5'];
            $sEmail = strtolower($sEmail);

            $sQry = "SELECT * FROM User WHERE Email='{$sEmail}' AND PwdMd5='{$sPasswordOld}' LIMIT 1";
            if(($oRes = $this->oDb->query($sQry)) === false) { /* Error */ return false; }
            if(!$oRes->num_rows) { /* Error */ return false; }
            if(!$aRow = $oRes->fetch_assoc()) { /* Error */ return false; }
            $aUserInfo = $aRow;
            ...
            $sQry = "UPDATE User SET PwdMd5='{$sPassword}' WHERE Id={$aUserInfo['Id']} LIMIT 1";
            if(($oRes = $this->oDb->query($sQry)) === false) { /* Error */ return false; }
